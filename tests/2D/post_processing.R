...
for (file_test in file_test_vect) {
  ...
  for (i in 1:n_reps) {
    ...
    if(mode_MV == "PLS-R") {
      names <- c(names, "Beta1_locs", "Beta2_locs", "Beta1", "Beta2")
    }
    for (name in names) {
...
if(mode_MV == "PLS-R") {
  ## Beta 1
  data_plot <- rmses[["Beta1_locs"]][rmses[["Beta1_locs"]]$Group == "4", ]
  title_vect <- paste(
    "Beta1 RMSE at locations w.r.t the",
    name_group_labels_vect
  )
  limits <- NULL
  source(paste("tests/", test_suite, "/templates/plot_overall.R", sep = ""))
  ## Beta 2
  data_plot <- rmses[["Beta2_locs"]][rmses[["Beta2_locs"]]$Group == "4", ]
  title_vect <- paste(
    "Beta2 RMSE at locations w.r.t the",
    name_group_labels_vect
  )
  limits <- NULL
  source(paste("tests/", test_suite, "/templates/plot_overall.R", sep = ""))
...
if(mode_MV == "PLS-R") {
  ## Beta 1
  data_plot <- rmses[["Beta1"]][rmses[["Beta1"]]$Group == "4", ]
  title_vect <- paste(
    "Beta1 RMSE w.r.t the",
    name_group_labels_vect
  )
  limits <- NULL
  source(paste("tests/", test_suite, "/templates/plot_overall.R", sep = ""))
  ## Beta 2
  data_plot <- rmses[["Beta2"]][rmses[["Beta2"]]$Group == "4", ]
  title_vect <- paste(
    "Beta2 RMSE w.r.t the",
    name_group_labels_vect
  )
  limits <- NULL
  source(paste("tests/", test_suite, "/templates/plot_overall.R", sep = ""))
...
if(name_main_test == "test3") {
  ...
  pdf(paste(path_images, "paper.pdf", sep = ""), width = 10, height = 5)
  ...
  data <- rmses$Y_reconstruction[index_NSR_Y & index_NSR_X & index_group, c(2,7, 9)]
  data_colnames <- colnames(data)
  data_colnames[1] <- "Group"
  colnames(data) <- data_colnames
  plot.grouped_boxplots(data, group_name = "",
                        subgroup_colors = colors[c(1, 3)], values_name = "",
                        subgroup_labels = lables_models[c(1, 3)]) + standard_plot_settings()
  
  
  index_NSR_Y <- rmses$X_reconstruction_locs$NSR_Y == 0.1
  index_NSR_X <- rmses$X_reconstruction_locs$NSR_X == 3
  data <- rmses$X_reconstruction_locs[index_NSR_Y & index_NSR_X & index_group, c(2,7, 9)]
  data_colnames <- colnames(data)
  data_colnames[1] <- "Group"
  colnames(data) <- data_colnames
  plot.grouped_boxplots(data, group_name = "",
  ...
  index_NSR_Y <- rmses$Beta1_locs$NSR_Y == 0.1
  index_NSR_X <- rmses$Beta1_locs$NSR_X == 3
  data <- rmses$Beta1_locs[index_NSR_Y & index_NSR_X & index_group, c(2, 7, 9)]
  data_colnames <- colnames(data)
  data_colnames[1] <- "Group"
  colnames(data) <- data_colnames
  plot.grouped_boxplots(data, group_name = "",
  ...
  index_NSR_Y <- rmses$Beta2_locs$NSR_Y == 0.1
  index_NSR_X <- rmses$Beta2_locs$NSR_X == 3
  data <- rmses$Beta2_locs[index_NSR_Y & index_NSR_X & index_group, c(2, 7, 9)]
  data_colnames <- colnames(data)
  data_colnames[1] <- "Group"
  colnames(data) <- data_colnames
  plot.grouped_boxplots(data, group_name = "",
  ...
  pdf(paste(path_images, "paper.pdf", sep = ""), width = 3, height = 5)
  
  index_NSR_Y <- rmses$Y_reconstruction$NSR_Y == 0.1
  index_NSR_X <- rmses$Y_reconstruction$NSR_X == 3
  index_group <- rmses$Y_reconstruction$Group == 4
  data <- rmses$Y_reconstruction[index_NSR_Y & index_NSR_X & index_group, c(4, 7, 9)]
  data_colnames <- colnames(data)
  data_colnames[1] <- "Group"
  colnames(data) <- data_colnames
  plot.grouped_boxplots(data, group_name = "",
                        subgroup_colors = colors[c(1, 3)], values_name = "",
                        subgroup_labels = lables_models[c(1, 3)],
                        LEGEND = FALSE) + standard_plot_settings()
  
  
  index_NSR_Y <- rmses$X_reconstruction_locs$NSR_Y == 0.1
  index_NSR_X <- rmses$X_reconstruction_locs$NSR_X == 3
  data <- rmses$X_reconstruction_locs[index_NSR_Y & index_NSR_X & index_group, c(4 ,7, 9)]
  data_colnames <- colnames(data)
  data_colnames[1] <- "Group"
  colnames(data) <- data_colnames
  plot.grouped_boxplots(data, group_name = "",
  ...
  index_NSR_Y <- rmses$Beta1_locs$NSR_Y == 0.1
  index_NSR_X <- rmses$Beta1_locs$NSR_X == 3
  data <- rmses$Beta1_locs[index_NSR_Y & index_NSR_X & index_group, c(4, 7, 9)]
  data_colnames <- colnames(data)
  data_colnames[1] <- "Group"
  colnames(data) <- data_colnames
  plot.grouped_boxplots(data, group_name = "",
  ...
  index_NSR_Y <- rmses$Beta2_locs$NSR_Y == 0.1
  index_NSR_X <- rmses$Beta2_locs$NSR_X == 3
  data <- rmses$Beta2_locs[index_NSR_Y & index_NSR_X & index_group, c(4, 7, 9)]
  data_colnames <- colnames(data)
  data_colnames[1] <- "Group"
  colnames(data) <- data_colnames
  plot.grouped_boxplots(data, group_name = "",
  ...
  index_NSR_X <- rmses$Y_reconstruction$NSR_X == 3
  index_group <- rmses$Y_reconstruction$Group == 4
  data <- rmses$Y_reconstruction[index_NSR_Y & index_NSR_X & index_group, c(3, 7, 9)]
  data_colnames <- colnames(data)
  data_colnames[1] <- "Group"
  colnames(data) <- data_colnames
  plot.grouped_boxplots(data, group_name = "",
  ...
  index_NSR_Y <- rmses$X_reconstruction_locs$NSR_Y == 0.1
  index_NSR_X <- rmses$X_reconstruction_locs$NSR_X == 3
  data <- rmses$X_reconstruction_locs[index_NSR_Y & index_NSR_X & index_group, c(3 ,7, 9)]
  data_colnames <- colnames(data)
  data_colnames[1] <- "Group"
  colnames(data) <- data_colnames
  plot.grouped_boxplots(data, group_name = "",
  ...
  index_NSR_Y <- rmses$Beta1_locs$NSR_Y == 0.1
  index_NSR_X <- rmses$Beta1_locs$NSR_X == 3
  data <- rmses$Beta1_locs[index_NSR_Y & index_NSR_X & index_group, c(3, 7, 9)]
  data_colnames <- colnames(data)
  data_colnames[1] <- "Group"
  colnames(data) <- data_colnames
  plot.grouped_boxplots(data, group_name = "",
  ...
  index_NSR_Y <- rmses$Beta2_locs$NSR_Y == 0.1
  index_NSR_X <- rmses$Beta2_locs$NSR_X == 3
  data <- rmses$Beta2_locs[index_NSR_Y & index_NSR_X & index_group, c(3, 7, 9)]
  data_colnames <- colnames(data)
  data_colnames[1] <- "Group"
  colnames(data) <- data_colnames
  plot.grouped_boxplots(data, group_name = "",
